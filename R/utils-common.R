...
#'
#' @param subjects Data frame with columns id, D_star.
...
#' Fit a residual-time Cox model
#'
#' Fits a Cox proportional hazards model for residual survival time.
#'
#' @param subjects Data frame with columns U (follow-up time) and delta (event indicator).
#' @param x Numeric matrix of covariates.
#' @return A fitted model object used by \code{predict_residual_cox_risk}.
#' @export
fit_residual_cox <- function(subjects, x) {
  ...
  form <- stats::as.formula(paste("survival::Surv(time, status) ~", paste(names(df)[-(1:2)], collapse = "+")))
  fit <- tryCatch(
...
#' Predict risk from a residual Cox model
#'
#' @param model Fitted model from \code{fit_residual_cox}.
#' @param x_new Numeric matrix of new covariates.
#' @param horizons Numeric vector of prediction horizons.
#' @return Matrix of predicted risks (subjects x horizons).
#' @export
...
beran_state_covariates <- function(subjects, stage = NULL, include_x = TRUE,
  ...
  if (isTRUE(include_x)) {
    x <- cbind(x, base_covariates(subjects, covariate_specification))
  }
  x
}
